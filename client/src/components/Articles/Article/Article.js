...
import { HiUserCircle,HiOutlineThumbUp,HiOutlineTrash,HiThumbUp } from "react-icons/hi";
    ...
    const Likes = ()=>{
        if(article.likes.length>=0){
            return article.likes.find((like)=>like===(user?.result?._id)) ? (
                             <div className="flex items-center text-primary  cursor-pointer" onClick={()=>{dispatch(likeArticle(article._id))}}>   
                                     <HiThumbUp className="w-[20px] h-[20px] mr-[5px]"/>
                                     <p className="text-primary">Like {article.likes.length}</p>
                               </div>
                ):(
                    <div className="flex items-center hover:text-primary cursor-pointer" onClick={()=>{dispatch(likeArticle(article._id))}}>
        ...
        <>
            ...
            <div className = "bg-white shadow-md w-[300px] rounded-lg overflow-hidden m-2">
               ...
               {(user?.result?._id===article?.author)&&(
                  <button onClick={()=>{setCurrentId(article._id)}} className="absolute top-1 left-3">
                      <BsPencilSquare className="text-primary text-[30px]"/>
                 </button>
                )}
                   <img src= {article.selectedFile} className="w-full h-full" alt="Article"/>
               </div>
               <div className="py-2.5 px-4">
                      ...
                      <h4 className="text-sm text-primary my-2.5 mx-0">{article.title}</h4>
                      ...
                      <div className="flex justify-between items-center mt-[10px] py-[10px] px-0 border-t-[1px] border-gray-500 border-solid  ">
                               ...
                               {(user?.result?._id===article?.author)&&(
                                   <div className="flex items-center hover:text-red-500 cursor-pointer" onClick={()=>{dispatch(deleteArticle(article._id))}}>
